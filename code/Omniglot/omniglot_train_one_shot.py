...
def main():
    ...
    feature_encoder_optim = torch.optim.SGD(feature_encoder.parameters(),lr=LEARNING_RATE)  
    feature_encoder_scheduler = StepLR(feature_encoder_optim,step_size=100000,gamma=0.5)  
    coarse_relation_network_optim = torch.optim.SGD(coarse_relation_network.parameters(),lr=LEARNING_RATE)
    ...
    for episode in range(EPISODE):
        ...
        if (episode+1)%5000 == 0:
            ...
            for test_episode in range(TEST_EPISODE):
                ...
                relation_pairs = torch.cat((sample_features_ext,test_features_ext),2).view(-1,FEATURE_DIM*2,2,2)
                fine_relations = fine_relation_network(relation_pairs).view(-1,CLASS_NUM)
                fine_relations = F.softmax(fine_relations,dim=1)
